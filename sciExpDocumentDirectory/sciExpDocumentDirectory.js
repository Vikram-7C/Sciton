...
export default class sciExpDocumentDirectory extends NavigationMixin(LightningElement) {
    ...
    handleNodeSelect(event) {
        this.selectedNode = event.detail.name;
    ...
    previewVideo(event) {
        ...
        if (selectedId) {
            ...
            getVideoContentVersions({ contentDocumentId: selectedId })
                .then(result => {
                    const videoOptions = Object.keys(result).map(item => ({
                        label: result[item],
                        value: item,
            ...
            console.error('Selected video ID is undefined.');
        }
    }
